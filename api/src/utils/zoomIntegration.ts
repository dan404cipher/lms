    ...
    try {
      ...
      for (const recording of recordings) {
        try {
          ...
          // Filter recording files to only include actual video files with both audio and video
          const videoFiles = (recording.recording_files || []).filter((file: any) => {
            // Only process MP4 files (video with audio)
            // Exclude M4A (audio only), CHAT, TRANSCRIPT, TIMELINE, etc.
            if (file.file_type !== 'MP4') {
              console.log(`⏭️  Skipping non-video file: ${file.file_type} (${file.recording_type || 'unknown type'})`);
              return false;
            }
            
            // Exclude audio-only recordings even if they're MP4
            if (file.recording_type === 'audio_only') {
              console.log(`⏭️  Skipping audio-only file: ${file.id}`);
              return false;
            }
            
            // Only include video files with actual video content
            return true;
          });

          // Prioritize recording types: shared_screen_with_speaker_view > gallery_view > active_speaker > speaker_view
          const priorityOrder = ['shared_screen_with_speaker_view', 'gallery_view', 'active_speaker', 'speaker_view', 'shared_screen'];
          videoFiles.sort((a: any, b: any) => {
            const aIndex = priorityOrder.indexOf(a.recording_type || '');
            const bIndex = priorityOrder.indexOf(b.recording_type || '');
            return (aIndex === -1 ? 999 : aIndex) - (bIndex === -1 ? 999 : bIndex);
          });

          console.log(`📹 Found ${videoFiles.length} video file(s) out of ${recording.recording_files?.length || 0} total files`);

          // Process only the video files
          for (const file of videoFiles) {
            try {
              console.log(`📹 Processing video file: ${file.file_type} (${file.recording_type || 'standard'}) - Size: ${(file.file_size / 1024 / 1024).toFixed(2)}MB`);
              ...
              const recordingData = {
                ...
                title: `${recording.topic || session.title}${file.recording_type ? ` - ${file.recording_type.replace(/_/g, ' ')}` : ''}`,
                description: `Recording of ${session.title} session`,
                recordingUrl: localRecordingUrl, // Use local URL if available
                downloadUrl: file.download_url,
                playUrl: file.play_url, // Store play URL for video streaming
